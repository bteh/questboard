...
Profile config has `additional_sources`, but the implemented extra scraper today is YC Work at a Startup.
Greenhouse/Lever coverage currently means ATS detection for application links, not general board crawling.
The company classifier knows 50+ VC-backed startups (OpenAI, Anthropic, Stripe, dbt Labs, Vercel, etc.)
and classifies by funding stage/amount/employee count. Auto-apply detects Greenhouse/Lever from job URLs.
