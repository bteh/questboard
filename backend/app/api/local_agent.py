...
_AGENT_TASKS: dict[str, dict[str, object]] = {
    "find_and_rank": {
        ...
        "prompt": (
            ...
            "the whole run: never start a second pull, and never use refresh_work to check "
            ...
            "call search_work(queries=judged_roles) and rank what my board already holds. "
            "Retrieval order is not a fit verdict, so map each candidate against my actual "
            "background. Call set_work_fit IN BATCHES of about 15, best candidates first, "
            "until EVERY candidate search_work returned has its own verdict, so my whole "
            "board is scored, not just the top few. Batches of one run add up; each batch "
            "lands on my board as soon as you send it, so if the run is cut short I keep "
            "what you already decided. Do NOT wait and send them all at the end. AFTER your "
            "last batch, check the pull once with get_refresh_status(run_id from "
            "refresh_work): if it finished, call search_work(queries=judged_roles) again and "
            "give verdicts to just the candidates that do not have one yet, ranking them to "
            "CONTINUE after your highest rank so far, never reusing a number; if it is still "
