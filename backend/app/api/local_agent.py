...
_AGENT_TASKS: dict[str, dict[str, object]] = {
    ...
    "find_and_rank": {
        ...
        "prompt": (
            ...
            "run, then stop. Verdict tiers: "
            "strong = every stated must-have maps to my resume evidence, no level or domain "
            "gap; good = one gap in the must-haves, no level or domain stretch; reach = a "
            "level or domain stretch, or more than one gap; skip = not my work (wrong role, "
            "staffing agency, junk). Mark at most a third of the non-skips strong unless the "
            "evidence truly supports more; a run where one tier covers nearly everything is "
            "a failed ranking. Rank the non-skips 1..N best-first; skips "
